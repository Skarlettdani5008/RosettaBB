...
struct AppScannerTests {
    ...
    private func makeApp(named name: String, in root: URL, execBytes: [UInt8]) throws {
        let macOS = root
            .appendingPathComponent("\(name).app")
            .appendingPathComponent("Contents")
            .appendingPathComponent("MacOS")
        try FileManager.default.createDirectory(at: macOS, withIntermediateDirectories: true)
        // Info.plist с CFBundleExecutable
        let plist: [String: Any] = ["CFBundleExecutable": name, "CFBundleName": name]
        let plistURL = root
            .appendingPathComponent("\(name).app")
            .appendingPathComponent("Contents")
